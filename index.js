...
function setToArray(mySet) {
  ...
  let set = new Set(mySet);
  let array = [...set];
  let filterSet = array.filter((set) => set.toString());
  let sortSet = filterSet.sort();
  return sortSet;
}
...
function iterateSet(set) {
  ...
  let myKeys = set.keys();
  for (let keyss of myKeys) {
    console.log(keyss);
  }
  let myValues = set.values();
  for (let val of myValues) {
    console.log(val);
  }
  let myEntries = set.entries();
  for (let entri of myEntries) {
    console.log(entri);
  }
}
